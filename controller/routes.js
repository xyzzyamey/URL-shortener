...
router.get('/dashboard', checkAuth, (req, res) => {
    ...
    urls.find({ owned : req.user.email }, (err, data) => {
        ...
        res.render('dashboard', { verified: req.user.isVerified, logged: true, csrfToken: req.csrfToken(), urls : data, og:"" });
...
router.post('/create', checkAuth, (req, res) => {
    ...
    if (!original || !short) {
        ...
        urls.find({ owned : req.user.email }, (err, data) => {
            ...
            res.render('dashboard', { verified: req.user.isVerified, logged: true, csrfToken: req.csrfToken(), err: "Empty Fields !",  urls : data , og:""});
            ...
            urls.find({ owned : req.user.email }, (err, data) => {
                ...
                res.render('dashboard', { verified: req.user.isVerified, logged: true, csrfToken: req.csrfToken(), err: "Empty Fields !",  urls : data, og: original });
        ...
        urls.findOne({ slug: short }, (err, data) => {
            ...
            if (data) {
                urls.find({ owned : req.user.email }, (err, data) => {
                    ...
                    res.render('dashboard', { verified: req.user.isVerified, logged: true, csrfToken: req.csrfToken(), err: "Empty Fields !",err: "Try Different Short Url, This exists !",  urls : data, og: original });
...
router.get('/del', checkAuth, async(req, res)=>{
    console.log(req.query.slug)
    console.log(req.user.email)
    if (req.query.slug != undefined) {
        var data = await urls.findOne({$and : [{ slug: req.query.slug },{owned : req.user.email}]});
        if(data)
        {
            await urls.findOneAndDelete({ slug: req.query.slug });
        }
