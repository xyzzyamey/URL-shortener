...
router.get('/user/send-verification-email', checkAuth, async (req, res) => {
    ...
    if (req.user.isVerified || req.user.provider == 'google') {
        ...
        await urls.find({ owned : req.user.email }, (err, data) => {
            if(err) throw err;
            
            res.render('dashboard', { verified: req.user.isVerified, logged: true, csrfToken: req.csrfToken(), urls : data, emailsent: true, og:"" });
    ...
    if (token) {
        ...
        if (check) {
            ...
            await urls.find({ owned : req.user.email }, (err, data) => {
                ...
                res.render('dashboard', { verified: req.user.isVerified, logged: true, csrfToken: req.csrfToken(), urls : data, emailsent: true, err: "Invalid token or Token has expired, Try again.", og:"" });
...
router.post('/user/forgot-password', async (req, res) => {
    ...
    if (userData) {
        if (userData.provider == 'google') {
            ...
            res.render('forgot-password.ejs', { csrfToken: req.csrfToken(), msg: "User exists with Google account. Try logging using it.", type: 'danger' });
        } else {
            ...
            res.render('forgot-password.ejs', { csrfToken: req.csrfToken(), msg: "Reset email sent. Reset link is valid for 10 minutes. Check your email for more info.", type: 'success' });
        }
    } else {
